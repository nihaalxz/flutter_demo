import 'package:flutter/foundation.dart';
import 'package:geolocator/geolocator.dart';
import 'package:geocoding/geocoding.dart';
import 'dart:io' show Platform;

class LocationService {
  /// Determines the current position of the device.
  ///
  /// When location services are not enabled or permissions
  /// are denied the `Future` will return an error.
  static Future<Position> getCurrentPosition() async {
    bool serviceEnabled;
    LocationPermission permission;

    // Test if location services are enabled.
    serviceEnabled = await Geolocator.isLocationServiceEnabled();
    if (!serviceEnabled) {
      // Location services are not enabled don't continue
      // accessing the position and request users of the
      // App to enable the location services.
      return Future.error('Location services are disabled.');
    }

    permission = await Geolocator.checkPermission();
    if (permission == LocationPermission.denied) {
      permission = await Geolocator.requestPermission();
      if (permission == LocationPermission.denied) {
        // Permissions are denied, next time you could try
        // requesting permissions again (this is also where
        // Android's shouldShowRequestPermissionRationale
        // returned true. According to Android guidelines
        // your App should show an explanatory UI now.
        return Future.error('Location permissions are denied');
      }
    }

    if (permission == LocationPermission.deniedForever) {
      // Permissions are denied forever, handle appropriately.
      // We can open the app settings to allow the user to grant permission.
      await Geolocator.openAppSettings();
      return Future.error(
        'Location permissions are permanently denied, please enable it from settings.',
      );
    }
    
    if (kDebugMode) {
      print("[LocationService] Permission status: $permission");
    }

    // When we reach here, permissions are granted and we can
    // continue accessing the position of the device.

    // Define platform-specific settings for better performance/accuracy.
    final LocationSettings locationSettings = LocationSettings(
      accuracy: LocationAccuracy.high,
      distanceFilter: Platform.isIOS ? 100 : 0, // iOS can use distance filter
    );

    return await Geolocator.getCurrentPosition(
      locationSettings: locationSettings,
    );
  }

  /// Converts GPS coordinates into a human-readable city name.
  static Future<String> getCityFromCoordinates(Position position) async {
    try {
      List<Placemark> placemarks = await placemarkFromCoordinates(
        position.latitude,
        position.longitude,
      );

      if (placemarks.isNotEmpty) {
        // .locality is typically the city name.
        return placemarks.first.locality ?? "Unknown City";
      }
      return "Unknown City";
    } catch (e) {
      if (kDebugMode) {
        print("[LocationService] Error getting city: $e");
      }
      return "Could not determine city";
    }
  }
}
